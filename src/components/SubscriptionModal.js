...
class SubscriptionModal extends HTMLElement {
  constructor() {
    ...
    this._subscriptionConfirmed = false;
  ...
  async fetchNetworkFee() {
    ...
    if (this._subscriptionConfirmed) return;
    ...
    try {
      ...
      this._networkFee = { fee: "0.000000000000 POL", usdValue: "($0.00)" };
    } finally {
  ...
  async render() {
    ...
    const template = `
      <style>
        ...
        span[aria-live="polite"]:has(> span.buttons-loader) {
          width: 100%;
        }
        ...
        <div class="modal-container" id="modalContainer">
          ...
          <div class="modal-body">
            <div class="modal-body-container body-main ${
              this._showError || this._subscriptionConfirmed ? "hidden" : ""
            }">
              ...
              ${
                !this._isFeeLoading && modalData.needsDeposit
                  ? `
                <div class="notice-text">
                  <span>You need to deposit approximately</span>
                  <span class="detail-value small">
                    ${formatUnits(modalData.depositAmount, 6)}
                  </span>
                  <img src="${tokenIcon}" alt="Token Icon" class="token-icon-small" />
              ...
              <div class="buttons-section" id="buttonsSection">
                <!-- Approve and Subscribe buttons will be inserted here -->
              </div>
            </div>
            ${
              this._showError && !this._subscriptionConfirmed
                ? `
    ...
    if (buttonsSection) {
      if (this._isFeeLoading) {
        ...
        const subscribeBtn = createSubscribeButton({
          ...
          onSuccess: () => {
            this._subscriptionConfirmed = true;
